...
@section('content')
    ...
    <div class="mt-4">
        <div class="d-flex justify-content-between mb-4">
            ...
            @include('backend.school_admin.school_house.partials.action')
        </div>
        <div class="card">
            <div class="card-body">
        ...
        <div class="modal fade" id="createSchoolHouse" tabindex="-1" aria-labelledby="exampleModalLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Add School House</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="">
                        <form method="post" id="sectionForm" action="{{ route('admin.school-houses.store') }}">
                            @csrf
                            <input type="hidden" name="_method" id="methodField" value="POST">
                            <input type="hidden" name="dynamic_id" id="dynamic_id">
                            <div class="col-md-12">
                                <div class="p-2 label-input">
                                    <label>House Name<span class="must">*</span></label>
                                ...
                                <div class="p-2 label-input">
                                    ...
                                    <div class="single-input-modal">
                                        <textarea type="text" value="{{ old('description') }}" name="description" class="input-text single-input-text"
                                            id="dynamic_description" autofocus required>
                                        </textarea>
                                    </div>
                                </div>
                                <div class="p-2 label-input">
...
@section('scripts')
    <script>
        ...
        $('#section-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('admin.school-houses.get') }}',
                type: 'POST'
            },
            columns: [{
                    data: 'id',
        ...
        $(document).on('click', '.edit-schoolhouse', function() {
            var id = $(this).data('id');
            ...
            $('#sectionForm').attr('action', '{{ route('admin.school-houses.update', '') }}' + '/' + id);
            $('#methodField').val('PUT');

            $('#createSchoolHouse').modal('show');
