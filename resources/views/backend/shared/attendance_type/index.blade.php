...
@section('content')
    <div class="mt-4">
        <div class="d-flex justify-content-between mb-4">
            ...
            @include('backend.shared.attendance_type.partials.action')
        </div>
        <div class="card">
            <div class="card-body">
        ...
        <div class="modal fade" id="createAttendanceType" tabindex="-1" aria-labelledby="exampleModalLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Add Attendance Type</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="">
                        <form method="post" id="attendanceTypeForm" action="{{ route('admin.attendance-types.store') }}">
                            @csrf
...
@section('scripts')
    <script>
        ...
        $('#attendance-type-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: '{{ route('admin.attendance-types.get') }}',
                type: 'POST'
            },
            columns: [{
                    data: 'id',
        ...
        $(document).on('click', '.edit-attendance-type', function() {
            ...
            $('#attendanceTypeForm').attr('action', '{{ route('admin.attendance-types.update', '') }}' +
                '/' + id);
            $('#methodField').val('PUT');
