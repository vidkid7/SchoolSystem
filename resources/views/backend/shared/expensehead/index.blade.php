...
@section('content')
    ...
    <div class="mt-4">
        <div class="d-flex justify-content-between mb-4">
            ...
            @include('backend.shared.expensehead.partials.action')
        </div>
        <div class="card">
            <div class="card-body">
        ...
        <div class="modal fade" id="createExpensehead" tabindex="-1" aria-labelledby="exampleModalLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Add Expense Head</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-footer">
                        <form method="post" id="expenseHeadForm" action="{{ route('admin.expenses-head.store') }}">
                            @csrf
                            <input type="hidden" name="_method" id="methodField" value="POST">
                            <input type="hidden" name="dynamic_id" id="dynamic_id">
                            <div class="col-md-12 d-flex flex-wrap gap-5">
                                <div class="form-group col-md-4 col-xs-12">
                                    <label>Expense Category<span class="must">*</span></label>
                                    <div class="col-sm-10">
                                        <input type="text" value="{{ old('name') }}" name="name"
                                            class="input-text" id="dynamic_name" autofocus required>
                                    </div>
                                </div>
                                <div class="form-group col-md-4 col-xs-12">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" value="{{ old('description') }}" name="description"
                                            class="input-text" id="dynamic_description" required>
                                    </div>
                                </div>
                                <div class="form-group col-md-4 col-xs-12">
...
@section('scripts')
    <script>
        ...
        $('#expensehead-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('admin.expenses-head.get') }}',
                type: 'POST'
            },
            columns: [{
                    data: 'id',
        ...
        $(document).on('click', '.edit-expense-head', function() {
            ...
            $('#expenseHeadForm').attr('action', '{{ route('admin.expenses-head.update', '') }}' + '/' +
                id);
            $('#methodField').val('PUT');

            $('#createExpensehead').modal('show');
