...
@section('content')
    <div class="mt-4">
        <div class="d-flex justify-content-between mb-4">
            ...
            @include('backend.shared.extracurricularHead.partials.action')
        </div>
        <div class="card">
            <div class="card-body">
                        ...
                        <div class="col-sm-12 col-md-12 col-12">
                            <div class="report-table-container">
                                <div class="table-responsive">
                                    <table id="extracurricularhead-table"
        ...
        <div class="modal fade" id="createExtracurricularhead" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Add Extracurricular Head</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="">
                        <form method="post" id="extracurricularHeadForm" action="{{ route('admin.extracurricular-head.store') }}">
                            @csrf
                            <input type="hidden" name="_method" id="methodField" value="POST">
                            <input type="hidden" name="dynamic_id" id="dynamic_id">
                            <div class="col-md-12">
                                <div class="p-2 input-label">
                                    <label>ExtraCurricular Category<span class="must">*</span></label>
                                    <div class="single-input-modal">
                                        <input type="text" value="{{ old('name') }}" name="name"
                                            class="input-text single-input-text" id="dynamic_extracurricular_category" autofocus required>
                                    </div>
                                </div>
...
@section('scripts')
    <script>
        ...
        $('#extracurricularhead-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: '{{ route('admin.extracurricular-head.get') }}',
                type: 'POST'
            },
            columns: [{
                    data: 'id',
        ...
        $(document).on('click', '.edit-extracurricular-head', function() {
            ...
            $('#extracurricularHeadForm').attr('action', '{{ route('admin.extracurricular-head.update', '') }}' + '/' +
                id);
            $('#methodField').val('PUT');

            $('#createextracurricularhead').modal('show');
