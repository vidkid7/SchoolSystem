...
@section('scripts')
    <script>
        ...
        $('#municipality-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('admin.municipality-users.get') }}',
                type: 'post'
            },
            columns: [{
                    data: 'id',
        ...
        $(document).on('click', '.edit-designation', function() {
            ...
            $('#municipalityForm').attr('action', '{{ route('admin.municipality-users.update', '') }}' + '/' +
                id);
            $('#methodField').val('PUT');

            // Show the modal
            $('#createMunicipality').modal('show');
